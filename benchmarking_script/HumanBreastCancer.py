...
# sc.pl.spatial(adata_omics1, basis='spatial', color=adata_omics1.var_names[0], s=10, show=False)
...
# adata_omics1 = AgaeSMO_v1.construct_neighbor_graph_single(adata_omics1,6,k=6)

# In[20]:



model = AgaeSMO_v1.Train_AgaeSMO(data, device=device,learning_rate=0.001,
                             weight_decay=0,
                             epochs=args.epoch,
                             loss_weight=[1,1,1,1],
                             loss_fun="sce")
...
import pandas as pd
groundtruth=pd.read_csv(args.annotation_path,sep="\t",index_col=0)
# groundtruth.loc[groundtruth[1].isna(),:]="1"
adata.obs["groundtruth"]=groundtruth['ground_truth']

# sc.pl.embedding(adata, basis='spatial', color='groundtruth',  title='GroundTruth', s=100, show=False)
...
index=supervise_index(adata,'AgaeSMO','groundtruth')
print(args.save_label,index)
...
fig, ax = plt.subplots()
ARI=index["ARI"]
sc.pl.spatial(adata, basis='spatial', color='AgaeSMO',ax=ax ,title=f'AgaeSMO{ARI}', s=10, show=False)

ax.set_xlabel(f"ARI:{ARI}")
fig.savefig(args.save_path+args.save_label+"_AgaeSMO"+f"_{ARI}"+".jpg")
# In[ ]:

adata.write(args.save_path+args.save_label+"_AgaeSMO"+".h5ad")
